...
def es_connection(es_config):
    ...
    return ElasticInput(config=es_config, credentials=credentials)
...
def test_es_bad_credentials(es_config):
    ...
    bad_es_conn = ElasticInput(config=es_config, credentials=("test","badpassword"))

    events = bad_es_conn.main()
    assert events is not None
    assert len(events) == 0
