...
def tests(session):
    ...
    session.run("genbadge", "coverage", "-i", f"{REPORTS_DIR}/coverage.xml", "-o", f"{BAGDES_DIR}/coverage-badge.svg")
...
def lint(session):
    ...
    session.run("flake8", ".", "--exit-zero", "--statistics", f"--output-file={REPORTS_DIR}/flake8/flake8stats.txt")
